...
//            }
    ...
    private func sendMessageWithImageUrl(_ imageUrl: String,_ image: UIImage) {
        ...
        guard let toID = self.userid else {return}
    ...
    private func setUpCell(cell: ChatMessageCell, message: messageInfo) {
        ...
        guard let numberplate = numberplate else {return}

        let ref = Firestore.firestore().collection("users").document(chatPartnerID).collection("cars").document(numberplate)
        ...
        if message.FromID == "6qjvaNBtSeJySgQbgCOI" {
            //outgoing message
